...
ErrVal resolve_package_paths_repositories(
    ...
    for (size_t r = 0; r < n_repositories; r++) {
        ...
        for (size_t e = 0; e < n_entries; e++) {
            ...
            ResolvedPackage *brp;
            if (!llrb_char_ptr_resolvedpackage_get_ref(
                    resolved_packages,
                    &entrypackagename,
                    &brp
                )) {
                ...
                if (apk_version_compare(APK_BLOB_STR(entryversion), APK_BLOB_STR(brp->version))
                    == APK_VERSION_GREATER) {
    ...
    if (!insert_all) {
        ...
        for (size_t i = 0; i < vec_char_ptr_len(&todelete); i++) {
            ...
            bool removed =
                llrb_char_ptr_resolvedpackage_remove(resolved_packages, &key, NULL, &value);
            assert(removed);
            free(key);
